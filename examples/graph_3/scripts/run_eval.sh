...
# STEPBYSTEP=1: eval the stepbystep cartridges (train_stepbystep/) and add the
# step-by-step directive to the ICL prompt; results written under eval_stepbystep/.
SBS_ARGS=(); TRAINSUB="train"; EVALSUB="eval"; RES_SUFFIX=""
if [ "${STEPBYSTEP:-0}" = "1" ]; then
  SBS_ARGS=(--stepbystep); TRAINSUB="train_stepbystep"; EVALSUB="eval_stepbystep"
  RES_SUFFIX="_stepbystep"
fi
...
if [ "${SKIP_exp0:-0}" = "1" ]; then
  ...
  python -m examples.graph_3.evaluation.eval \
    ...
    "${THINK_ARGS[@]}" "${SBS_ARGS[@]}" "${LIMIT_ARGS[@]}" \
    --output "$OUT/exp0_icl/results${RES_SUFFIX}.json"
fi

# ── Exp 1 — adaptive-hint cartridge ───────────────────────────────────────────
EXP1_CKPT="$(find_ckpt "$OUT/exp1_adaptive/$TRAINSUB")"
if [ "${SKIP_exp1:-0}" = "1" ]; then
  echo "── SKIP exp1 ──"
elif [ -z "$EXP1_CKPT" ]; then
  echo "!! exp1: no checkpoint under $OUT/exp1_adaptive/$TRAINSUB — skipping"
else
  ...
  python -m examples.graph_3.evaluation.eval \
    ...
    --output "$OUT/exp1_adaptive/$EVALSUB/results.json"
fi

# ── Exp 2 — plain self-study cartridge ────────────────────────────────────────
EXP2_CKPT="$(find_ckpt "$OUT/exp2_plain/$TRAINSUB")"
if [ "${SKIP_exp2:-0}" = "1" ]; then
  echo "── SKIP exp2 ──"
elif [ -z "$EXP2_CKPT" ]; then
  echo "!! exp2: no checkpoint under $OUT/exp2_plain/$TRAINSUB — skipping"
else
  ...
  python -m examples.graph_3.evaluation.eval \
    ...
    "${THINK_ARGS[@]}" "${LIMIT_ARGS[@]}" \
    --output "$OUT/exp2_plain/$EVALSUB/results.json"
fi

# ── Exp 3 — rejection-sampled cartridge ───────────────────────────────────────
EXP3_CKPT="$(find_ckpt "$OUT/exp3_rejection/$TRAINSUB")"
if [ "${SKIP_exp3:-0}" = "1" ]; then
  echo "── SKIP exp3 ──"
elif [ -z "$EXP3_CKPT" ]; then
  echo "!! exp3: no checkpoint under $OUT/exp3_rejection/$TRAINSUB — skipping"
else
  ...
  python -m examples.graph_3.evaluation.eval \
    ...
    "${THINK_ARGS[@]}" "${LIMIT_ARGS[@]}" \
    --output "$OUT/exp3_rejection/$EVALSUB/results.json"
fi

# ── Comparison ────────────────────────────────────────────────────────────────
echo ""; echo "══ comparison ══"
FILES=()
[ -f "$OUT/exp0_icl/results${RES_SUFFIX}.json" ]      && FILES+=("$OUT/exp0_icl/results${RES_SUFFIX}.json")
[ -f "$OUT/exp1_adaptive/$EVALSUB/results.json" ] && FILES+=("$OUT/exp1_adaptive/$EVALSUB/results.json")
[ -f "$OUT/exp2_plain/$EVALSUB/results.json" ]   && FILES+=("$OUT/exp2_plain/$EVALSUB/results.json")
[ -f "$OUT/exp3_rejection/$EVALSUB/results.json" ] && FILES+=("$OUT/exp3_rejection/$EVALSUB/results.json")
